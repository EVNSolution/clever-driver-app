...
비밀번호 재설정은 관리자가 발급한 30분 유효 일회용 HTTPS 링크를 DSV 웹에서
여는 방식이다. 앱은 로그인 화면에서 이 발급 경로만 안내하며 SMS 인증,
초기화 토큰 수신, 재설정 API 호출을 구현하지 않는다. 재설정 완료로 기존
refresh session이 만료되면 기존 세션 만료 처리와 동일하게 SecureStore의
refresh token을 삭제한다.

회원가입은 로그인 화면의 버튼으로 진입한다. 앱은 이름과 휴대전화 번호를 서버에 보내고,
서버가 같은 이름과 정규화된 전화번호를 가진 활성·미연결 DSV 배송원만 원자적으로
연결한다. 가입 요청에는 사용자가 입력한 로그인 아이디 또는 이메일 주소와 비밀번호를 함께 전송하며,
별도 가입 토큰이나 주민등록정보는 수집하거나 전송하지 않는다.
