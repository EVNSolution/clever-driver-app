...
배송 정보는 새 하단 내비게이션 탭을 만들지 않고 `배송` 카드의 배송지명·주소
영역에서 여는 하단 시트로 제공한다. 시트 상단의 `주문 정보`와 `배송지 정보`
탭으로 해당 배송지의 개별 주문 정보와 배송지 공통 정보를 구분하며, 처음 열면
`주문 정보`를 표시한다. 목록 카드는 별도로 펼치지 않으며, 배송지명·주소 영역만
배송 정보 시트를 여는 조작 요소로 사용한다.
...
- 앱 다이얼로그: 화면 중앙 흰색 카드, 24px 모서리, 어두운 반투명 배경,
  ...
  빨간색, 취소는 회색으로 구분한다.
- 배송 정보 하단 시트: 기존 배송 증빙 시트와 같은 반투명 배경, 상단 핸들,
  둥근 상단 모서리를 사용한다. 헤더 아래에는 `주문 정보`와 `배송지 정보`의
  2개 탭을 두고 선택된 탭을 파란색으로 구분한다. 입력 항목별 마지막 수정일은
  라벨 바로 아래에 작은 보조문으로 표시한다.
...
- 배송 정보 하단 시트: 기본 `주문 정보` 탭에는 주문 수와 총 박스 수, 주문별
  운송조건과 박스 수를 제공한다. `배송지 정보` 탭에는 일반 메모, 점심시간과
  입장 여부를 묶은 입력 그룹, 필수 도착 시간과 각 항목의 마지막 수정일을
  제공한다. 점심시간은 `시작`과
  `종료`의 작은 숫자 입력을 `~`로 구분하고, 같은 줄 오른쪽에 제목 없는
  `가능/불가` 토글을 둔다. 필수 도착 시간도 짧은 폭의 숫자 입력을 사용한다.
  입력값은 `1200 → 12:00`처럼 자동 표시하지만 정상 상태의 설명 문구는 노출하지
  않고, 사용자가 `:`를 입력하게 하지 않는다. 플레이스홀더는 결과 형식이 보이도록
  `12:00`, `13:00`, `13:30`처럼 `:`를 포함한다.
...
- 앱 다이얼로그는 `alert` 역할과 modal 접근성 범위를 제공하고 모든 버튼은
  최소 44pt 터치 영역을 유지한다.
- 배송지명·주소 터치 영역은 `button` 역할과 명시적인 "배송 정보 열기" 라벨을
  갖는다. 우측 운송조건·박스 수는 조작할 수 없는 요약으로 표시한다. 하단 시트의
  두 탭은 `tab` 역할과 선택 상태를 제공한다.
- 모든 입력 시트는 키보드가 열린 높이를 반영해 스크롤 영역을 줄이고, 포커스된
  입력칸과 오류 문구가 키보드 및 고정 저장 버튼 위에 보이도록 자동 스크롤한다.
...
- 시스템 권한 요청: OS 권한 창을 그대로 사용하고, 거부 결과와 설정 이동 안내는
  앱 다이얼로그로 표시한다.
- 배송 정보 열기: 배송 카드의 배송지명·주소 영역을 누르면 하단 시트의
  `주문 정보` 탭을 연다. `배송지 정보` 탭으로 전환하면 배송지 공통 정보를
  확인하고 수정할 수 있다. 목록 카드 자체와 우측 박스 수 영역은 펼쳐지거나
  별도 화면을 열지 않는다.
