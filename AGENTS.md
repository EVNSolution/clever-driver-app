...
- 로그인 아이디와 비밀번호를 사용하고, 가입 기본 입력은 이름과 휴대전화 번호로
  유지한다.
- 신규 가입은 로그인 화면의 `회원가입` 버튼으로 시작하며 일회용 초대 링크를
  발급하거나 요구하지 않는다. 구버전 딥링크와 초대 검증 API는 이전 설치본의
  호환 경로일 뿐 현행 가입 절차나 운영 안내의 근거로 사용하지 않는다.
