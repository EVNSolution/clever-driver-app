...
`주문 정보`를 표시한다. 목록 카드의 주문 상세 펼치기와 배송 정보 열기는 서로
다른 터치 영역으로 구분한다.
...
- 배송지 정보 UI Preview의 배송일은 `2026-08-18`로 표시하고, 더미 주문의 순서
  편집 결과는 화면 메모리에만 유지해 서버 주문 상태로 전달하지 않는다.
...
- 배송지명·주소 터치 영역은 `button` 역할과 명시적인 "배송 정보 열기" 라벨을
  갖고, 주문 상세 펼치기 버튼과 중첩하지 않는다. 하단 시트의 두 탭은 `tab`
  역할과 선택 상태를 제공한다.
