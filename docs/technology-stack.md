...
회원가입은 `clever-driver://signup?token=...` 링크의 일회용 보안 토큰을 서버에서
검증한 경우에만 화면을 연다. 배송원 정보가 없는 초대에서는 가입자가 이름과 휴대전화
번호를 직접 입력하고 서버가 계정과 DSV 배송원을 원자적으로 생성한다. 앱은 시작 URL과 실행 중 URL 이벤트를 모두 처리하지만
토큰을 기기에 저장하거나 자체적으로 가입 권한을 판정하지 않는다. 토큰의 만료·폐기·
일회성 소비와 정확한 DSV 배송원 연결은 서버가 원자적으로 처리한다.
